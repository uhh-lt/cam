...
def add_points(contained_aspects, winner, sentence_score, sentence, max_sentscore, classification_confidence):
    if contained_aspects:
        if len(contained_aspects) == 1:
            winner.add_points(contained_aspects[0].name, (
                sentence_score / max_sentscore) * contained_aspects[0].weight * max_sentscore)
            winner.add_sentence(contained_aspects[0].name, [
                                classification_confidence, sentence])
        else:
            for aspect in contained_aspects:
                winner.add_points(
                    'multiple', (sentence_score / max_sentscore) * aspect.weight * max_sentscore)
            winner.add_sentence(
                'multiple', [classification_confidence, sentence])
    else:
        winner.add_points('none', sentence_score / max_sentscore)
