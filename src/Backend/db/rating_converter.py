    ...
    with open(CONVERTED_RATINGS_FILE_NAME, 'w') as target_file:
        for pair in rating_dict.keys():
            ...
            for aspect in rating_dict[pair].keys():
                good_dict[aspect] = rating_dict[pair][aspect].count(1)
            bad_dict = {}
            for aspect in rating_dict[pair].keys():
                bad_dict[aspect] = rating_dict[pair][aspect].count(0)
            average_dict = {}
            for aspect in rating_dict[pair].keys():
                average_dict[aspect] = sum(
                    rating_dict[pair][aspect]) / float(len(rating_dict[pair][aspect]))
            ...
            for aspect in sorted(average_dict, key=average_dict.__getitem__, reverse=True):
                target_file.write(
                    aspect + ';' + str(average_dict[aspect]) + '\n')
            target_file.write('\n\n')
