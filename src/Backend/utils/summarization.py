...
def find_most_frequent_words(object_a, object_b, sentences, aspect):
    ...
    for sentence in sentences:
        ...
        frequent_words['0_' + label].extend(get_words_between(
            word_list, tag_list, -1, indices[0][1], object_a, object_b))
        frequent_words['1_' + label].extend(get_words_between(
            word_list, tag_list, indices[0][1], indices[1][1], object_a, object_b))
        frequent_words['2_' + label].extend(get_words_between(
            word_list, tag_list, indices[1][1], indices[2][1], object_a, object_b))
        frequent_words['3_' + label].extend(get_words_between(
            word_list, tag_list, indices[2][1], len(word_list), object_a, object_b))
...
def print_frequent_words(frequent_words, object_a, object_b, aspect, labels):
    for label in labels:
        if label == 'a_c_b':
            print(frequent_words['0_'+label], '...', object_a, '...', frequent_words['1_'+label], '...',
                  aspect, '...', frequent_words['2_'+label], '...', object_b, '...', frequent_words['3_'+label])
        elif label == 'a_b_c':
            print(frequent_words['0_'+label], '...', object_a, '...', frequent_words['1_'+label], '...',
                  object_b, '...', frequent_words['2_'+label], '...', aspect, '...', frequent_words['3_'+label])
        elif label == 'b_a_c':
            print(frequent_words['0_'+label], '...', object_b, '...', frequent_words['1_'+label], '...',
                  object_a, '...', frequent_words['2_'+label], '...', aspect, '...', frequent_words['3_'+label])
        elif label == 'b_c_a':
            print(frequent_words['0_'+label], '...', object_b, '...', frequent_words['1_'+label], '...',
                  aspect, '...', frequent_words['2_'+label], '...', object_a, '...', frequent_words['3_'+label])
        elif label == 'c_a_b':
            print(frequent_words['0_'+label], '...', aspect, '...', frequent_words['1_'+label], '...',
                  object_a, '...', frequent_words['2_'+label], '...', object_b, '...', frequent_words['3_'+label])
        elif label == 'c_b_a':
            print(frequent_words['0_'+label], '...', aspect, '...', frequent_words['1_'+label], '...',
                  object_b, '...', frequent_words['2_'+label], '...', object_a, '...', frequent_words['3_'+label])
...
def find_most_frequent_comparative_word(object_a, object_b, sentences, aspect):
    ...
    for sentence in sentences:
        ...
        word_list = word_tokenize(sentence)
        tag_list = pos_tag(word_list)
        comparative_words.extend(get_words_between(
            word_list, tag_list, -1, len(word_list), object_a, object_b))
...
# nlp = spacy.load('en')
# doc = nlp('A has bigger mass than B.' \
#     'and venus has lower mass than earth and 90 bars of co2 pressure and it is 500 degress hot, why 1 mil.' \
#     'the diameter of venus is 12, 092 km ( sole 650 km less than the earth \' s ) and its mass is 81. 5 % of the earth \' s.')
# for token in doc:
#     if token.dep_ in ['appos', 'amod', 'acomp']:
