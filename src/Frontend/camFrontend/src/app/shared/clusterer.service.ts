...
export class ClustererService {
  ...
  /**
     ...
     */
  getCluster(sentence, winner_links, loser_links, finalAspDict, object_A, object_B) {
    const wordList = sentence.match(/([A-Za-z]+)/g); // extract the words
    // list containing all the words to be highlighted
    const highlightList = winner_links.concat(loser_links).concat(Object.keys(finalAspDict));
